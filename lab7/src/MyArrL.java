...
public class MyArrL <T> {
    ...
    public MyArrL(int size) {
        this.size = size;
        l = (T[]) new Object[size];
        count = 0;
    }
    public MyArrL(MyArrL<T> other) {
    ...
    public boolean addAll(int index, MyArrL<T> other) {
    ...
    public MyArrL<T> subList(int start, int end) {
        MyArrL<T> nl = new MyArrL<>();
    ...
    public String toString() {
        return "Лист: " + "" + Arrays.toString(l) + ", Количество значений: " + count + ", Размер: " + size;
